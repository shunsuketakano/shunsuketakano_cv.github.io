...
物理量を基底で展開する際，活性表現の（基底函数$$\varphi_{\Gamma_0}^i$$ではなく）展開係数$$c_{\Gamma_0}^i$$の組みが秩序変数である．なお，活性表現$$\Gamma_0$$は一意とは限らないが，Landau理論を議論する場合には適切なひとつを選択する．ある流儀によると相転移では特定の既約表現のみが生き残ると考え，別の流儀では基底函数の固有値が最小のものをとる．この疑問はopen questionにしておき，以下ではうまくいく選択を例示することにする．また，対称操作により基底函数が変換するのか，それとも係数である秩序変数自身が変換するのかについては，流儀がある．秩序変数を変換する {% cite 犬井1985応用群論 上江州2016 %} ことも理解できるが，ここでは基底函数の変換に統一することで読者の混乱を避ける．熱力学の立場からみた秩序変数については {% cite 清水2021熱力学II %} の解説が納得である．

### 現象論的自由エネルギーの式形
（現象論的）自由エネルギー$$F$$を秩序変数$$\eta$$の函数として表す．熱平衡状態における秩序変数$$\eta^\ast$$により，$$F$$は最小化される（と仮定して式形を決める）．つまり，平衡値$$\eta^\ast$$から秩序変数$$\eta$$の値が外れたとき，$$F$$は最小値ではない（$$F(\eta) \ge F(\eta^\ast)$$）．ここで，自由エネルギーを含む完全な熱力学函数は熱平衡状態において定義されることから，平衡値でない$$\eta$$を変数にもつ$$F$$は``自由エネルギー''ではなく，ここでは``現象論的自由エネルギー''と称する．平衡値$$\eta=\eta^\ast$$における$$F(\eta^\ast) = \min_\eta F(\eta)$$は自由エネルギーである．自由エネルギーおよび現象論的自由エネルギーはスカラー量でなくてはならない．スカラー量とは，高温相の対称群$$G_0$$の全対称表現の基底函数である（と仮定する）．つまり，$$G$$のすべての対称要素$$g \in G_0$$について，$$F(\eta)$$は不変である：
