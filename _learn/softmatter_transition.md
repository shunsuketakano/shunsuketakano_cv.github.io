...
を得る．展開係数を制約する．係数$$A, B, C$$のいずれにも温度依存性がありうるが，相転移を記述するには$$A$$にのみ温度依存性を導入すればよい．具体的には，$$A(T^\ast) = 0, \frac{dA}{dT} > 0$$とする．さらに，現象論的自由エネルギーが$$S\ge0$$において最小値をもつために$$C > 0$$，相転移を引き起こすために$$B < 0$$を仮定する．なお，計算を簡単にするために不連続相転移を諦めて$$B = 0$$を仮定する場合もある．さて，スカラー秩序変数$$S$$の平衡値を求める．相転移温度$$T_0$$を，低温相$$S=0$$と高温相$$S > 0$$の現象論的自由エネルギーが一致する温度とする．このとき，$$f(S)$$は双安定となる．このことは，$$f$$を
...
である．これより低温でも高温相が存在しうるが，高温相が極小にもなり得ないとき，$$f \sim S^3 (S ~ 0)$$である．これは$$A(T^\ast) = 0$$である：
...
一方で，低温相が完全に消失する透明点$T^{\text{c}}$においては，$$S=0$$のみが極小になる．すなわち，
