...
\mathbf{M} = \mathbf{0}$$が平衡値となることを保障するために存在してはならない．また，4次項までがあれば相転移を記述できることから，5次以上の項は除外してある．いまの次数では連続相転移が現れるが，不連続相転移を記述するために6次項を導入する場合もある．なお，3次項は，秩序変数が空間的に一様と仮定する限りは現れない．展開係数は温度$$T$$に依存する．相転移近傍を記述する最低限のモデルでは，$$a(T^\ast) = 0, \frac{da}{dT} < 0, \frac{db}{dT} = 0$$を要求すればよい．
