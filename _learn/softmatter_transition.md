...
### 秩序変数の選択
物理量というものは，ある完全系を成す基底の線型結合で表される．特に，基底として，相の対称群における既約表現の基底函数の組み$$\{\varphi_{\Gamma_\alpha}^i\}$$を選ぶことができる．つまり，任意の物理量$$X$$は相の対称性適合基底で展開される：
...
ここに，$$\{c_{\Gamma_\alpha}^i\}$$は展開係数である．例えば，高温相における物理量は，$$G_0$$の既約表現の基底の線型結合となる．ここで，$$G_0$$の（全対称表現でない）既約表現が，低温相の既約表現に化ける場合がある．正確には，$$G_0$$における（全対称表現でない）既約表現$$\Gamma_0$$について，低温相の対称群$$G$$に制限することで$$\Gamma, \Gamma_1,\dots$$に既約分解され，さらに，その中に$$G$$の全対称表現が含まれていることがある．このような表現を「活性表現」という．物理量を基底で展開する際，活性表現の（基底函数$$\varepsilon_{\Gamma_0}^i$$ではなく）展開係数$$c_{\Gamma_0}^i$$の組みが秩序変数である．なお，活性表現$$\Gamma_0$$は一意とは限らないが，Landau理論を議論する場合には適切なひとつを選択する．ある流儀によると相転移では特定の既約表現のみが生き残ると考え，別の流儀では基底函数の固有値が最小のものをとる．この疑問はopen questionにしておき，以下ではうまくいく選択を例示することにする．熱力学の立場からみた秩序変数については {% cite 清水2021熱力学II %} の解説が納得である．
...
対称操作により，基底函数が変換するのか，それとも係数である秩序変数自身が変換するのかについては，流儀がある．秩序変数を変換する {% cite 犬井1985応用群論 %} ことも理解できるが，ここでは基底函数の変換に統一することで読者の混乱を避ける．
...
{% cite LandauLifshitzStat1980 上江州2016 de_Gennes %}
