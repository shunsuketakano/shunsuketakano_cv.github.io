...
を得る．これが，相転移点近傍における電気感受率である．ただし，低温相ではこの表式で実験結果を説明できない場合がある．それは，自発分極が生じると，それを抑制するような分域構造が（いま考えているよりも大きな空間スケールで）生じるためとされる{% cite 上江州2016 %}．自発分極のない高温相では分域の影響はなく，上記表式が適用できる．なお，分極機構として他の寄与がありうるため，その分を定数$$\chi_\infty$$として繰り込むことが多い．相転移温度近傍では$$a\simeq \frac{\partial a}{\partial T} (T - T^\ast)$$であることを仮定すると，高温相の感受率は，Curie–Weiss則
...
に従う．ここに，Curie定数$$C = \left(\varepsilon_0 \frac{\partial a}{\partial T}\right)^{-1}$$である．感受率$$\chi$$は相転移温度$$T^\ast$$に近づくにつれて発散的に増大する．これは，$$T^\ast$$付近において$$f$$の底が平たくなり，同じ印加電場でも大きく分極できるようになるためである．温度依存性の指数$$\gamma = 1$$は，Weissによる平均場近似の帰結と一致する．これは，Landau理論が平均場理論に相当することを示唆する．なお，低温相における自発分極は
